...
const ArraysUtils = /** @class */ (function () {
  ...
  ArraysUtils.uniq = function (arr) {
    return Array.from(new Set(__spreadArrays(arr)));
  };
  // Removes null, empty, undefined, falsy values from list
  ArraysUtils.compact = function (arr) {
    return arr.filter(Boolean);
  };
  ArraysUtils.reverse = function (arr) {
    return arr.reverse();
  };
  ArraysUtils.union = function (arr1, arr2) {
    return __spreadArrays(new Set(__spreadArrays(arr1, arr2)));
  };
  ArraysUtils.intersection = function (arr1, arr2) {
    return Array.from(
      new Set(
   ...
   * @param size
   */
  ArraysUtils.chunk = function (arr, size) {
...
var ObjectsUtils = /** @class */ (function () {
   ...
   * @param val
   */
  ObjectsUtils.is = function (type, val) {
    return ![, null].includes(val) && val.constructor === type;
  };
  ObjectsUtils.isObject = function (obj) {
    return !!obj && obj.constructor === {}.constructor;
  };
  ObjectsUtils.isFunction = function (val) {
   ...
   * @param val
   */
  ObjectsUtils.isNil = function (val) {
   ...
   * @param val
   */
  ObjectsUtils.isPrimitive = function (val) {
    return !['object', 'function'].includes(typeof val) || val === null;
  };
  ObjectsUtils.objectType = function (v) {
    return v === undefined
      ? 'undefined'
      : v === null
      ? 'null'
...
const StringsUtils = /** @class */ (function () {
   ...
   * @param text
   */
  StringsUtils.camelCase = function (text) {
    return text
      .replace(/\s(.)/g, ($1) => {
   ...
   * @param text
   */
  StringsUtils.pascalCase = function (text) {
    return text
   ...
   * @param string
   */
  StringsUtils.titleCase = function (text) {
    return text
      .split(' ')
      .map((w) => {
        return w.charAt(0).toUpperCase() + w.substr(1, w.length - 1);
      })
      .join(' ');
  };
  /**
   ...
   * @param text
   */
  StringsUtils.toggleCase = function (text) {
    return __spreadArrays(text)
      .map((c) => {
        return c.charCodeAt(0) <= 90 ? c.toLowerCase() : c.toUpperCase();
      })
      .join('');
  };
  StringsUtils.swapCase = function (text) {
    return StringsUtils.toggleCase(text);
  };
  /**
  ...
  StringsUtils.dotCase = function (text) {
    ...
    text = __spreadArrays(text)
      .map((w) => {
        return w.charCodeAt(0) <= 90 ? '.' + w.toLowerCase() : w;
      })
      .join('')
      .replace(/\.+/g, '.');
    return text;
   ...
   * @param text
   */
  StringsUtils.kebabCase = function (text) {
    return (
      text &&
      text
        .match(
   ...
   * @param text
   */
  StringsUtils.snakeCase = function (text) {
    return (
      text &&
      text
        .match(
   ...
   * @param num
   */
  StringsUtils.truncateString = function (str, num) {
  ...
  StringsUtils.isEmpty = function (val) {
  ...
  StringsUtils.isInteger = function (number) {
    return (number ^ 0) === number;
  };
  StringsUtils.isNumber = function (n) {
  ...
  StringsUtils.isLowerCase = function (str) {
    return str === str.toLowerCase();
  };
  StringsUtils.isUpperCase = function (str) {
   ...
   * @param str
   */
  StringsUtils.removeNonASCII = function (str) {
  ...
  StringsUtils.RGBToHex = function (r, g, b) {
...
const MathsUtils = /** @class */ (function () {
  function MathsUtils() {}
   ...
   * @param fn
   */
  MathsUtils.averageBy = function (arr, fn) {
    return (
      arr
        .map(
          typeof fn === 'function'
  ...
  MathsUtils.gcd = function () {
    ...
    const _gcd = function (x, y) {
      return !y ? x : exports.gcd(y, x % y);
    };
    return __spreadArrays(arr).reduce((a, b) => {
      return _gcd(a, b);
    });
  };
  /**
  ...
  MathsUtils.primes = function (num) {
    let arr = Array.from({length: num - 1}).map((x, i) => {
        return i + 2;
      }),
      sqroot = Math.floor(Math.sqrt(num)),
      numsTillSqroot = Array.from({length: sqroot - 1}).map((x, i) => {
        return i + 2;
      });
    numsTillSqroot.forEach((x) => {
      return (arr = arr.filter((y) => {
        return y % x !== 0 || y === x;
      }));
    });
    return arr;
  };
  MathsUtils.isEven = function (num) {
    return num % 2 === 0;
  };
  MathsUtils.isOdd = function (num) {
    return num % 2 === 1;
  };
  /**
   ...
   * @param fn
   */
  MathsUtils.maxBy = function (arr, fn) {
  ...
  MathsUtils.minBy = function (arr, fn) {
   ...
   * @param fn
   */
  MathsUtils.sumBy = function (arr, fn) {
    return arr
      .map(
        typeof fn === 'function'
          ? fn
  ...
  MathsUtils.median = function (arr) {
    const mid = Math.floor(arr.length / 2),
      nums = __spreadArrays(arr).sort((a, b) => {
        return a - b;
      });
   ...
   * @param val
   */
  MathsUtils.percentile = function (arr, val) {
    return (
      (100 *
        arr.reduce((acc, v) => {
   ...
   * @param num
   */
  MathsUtils.toSafeInteger = function (num) {
    return Math.round(
...
const ValidationsUtils = /** @class */ (function () {
  ...
  ValidationsUtils.isValidCitizenId = function (id, country) {
    ...
    switch (country) {
      ...
      case 'TR':
        ...
        var s = 0;
        var c = 0;
        for (let b_1 = 0; 10 > c; c++) s += parseInt(id.charAt(c), 10);
...
const PromiseUtil = /** @class */ (function () {
   ...
   * @param func
   */
  PromiseUtil.promisify = function (func) {
    return function () {
      const args = [];
      for (let _i = 0; _i < arguments.length; _i++) {
...
exports.Queue = Queue;
const LimitedQueue = /** @class */ (function (_super) {
  ...
  LimitedQueue.prototype.add = function (data) {
    ...
    while (this.size() > this.limit) _super.prototype.poll.call(this);
  };
  return LimitedQueue;
})(Queue);
