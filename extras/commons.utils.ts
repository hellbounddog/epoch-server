...
class ObjectsUtils {
  ...
  static safeparse = (item, defaultReturn?: any) => {
    // safe json parsing defaultReturn can be empty array or empty object ([], {})
    try {
...
class StringsUtils {
  ...
  static extractHyphenated = (text: string): string[] => {
    // Extract hyphenated text
    return text.match(/([a-zA-Z0-9]+-){1,}[a-zA-Z0-9]+/gi) || [];
  };

  static extractTime = (text: string): string[] => {
    // Extract time
    return text.match(/([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]/gi) || [];
  };

  static extractDotted = (text: string): string[] => {
    // E.A.D.P., Washington D.C. or L.A.
  ...
  static words = (text: string, pattern = /[^a-zA-Z-]+/) =>
...
class ValidationsUtils {
  ...
  static validateCreditCard = (cardnumber: string, cardtype = 'all') => {
...
export const extractUSCurrencyAndNumbers =
  StringsUtils.extractUSCurrencyAndNumbers;
