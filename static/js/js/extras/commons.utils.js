...
ArraysUtils.intersection = (arr1, arr2) => Array.from(new Set([...arr1].filter(x => arr2.indexOf(x) >= 0)));
 ...
 * @param size
 */
ArraysUtils.chunk = (arr, size) => Array.from({ length: Math.ceil(arr.length / size) }, (v, i) => arr.slice(i * size, i * size + size));
/**
 ...
 * @param n
 */
ArraysUtils.nthElement = (arr, n = 0) => (n === -1 ? arr.slice(n) : arr.slice(n, n + 1))[0];
class ObjectsUtils {
}
...
ObjectsUtils.isEmpty = (obj = {}) => Object.getOwnPropertyNames(obj).length === 0;
 ...
 * @param val
 */
ObjectsUtils.isPrimitive = val => !['object', 'function'].includes(typeof val) || val === null;
ObjectsUtils.objectType = v => v === undefined ? 'undefined' : v === null ? 'null' : v.constructor.name.toLowerCase();
/**
 * lowercaseKeys({NAME: 'Pujan}) --> {name: 'Pujan}
 * @param obj
 */
ObjectsUtils.lowercaseKeys = obj => Object.keys(obj).reduce((acc, key) => {
 ...
 * @param text
 */
StringsUtils.camelCase = (text) => text.replace(/\s(.)/g, ($1) => $1.toUpperCase()).replace(/\s/g, '');
/**
 * hello javascript => HelloJavascript
 *
 * @param text
 */
StringsUtils.pascalCase = (text) => text.replace(/(\w)(\w*)/g, (g0, g1, g2) => g1.toUpperCase() + g2.toLowerCase()).replace(/\s/g, '');
/**
 * hello javascript => Hello Javascript
 *
 * @param string
 */
StringsUtils.titleCase = (text) => text.split(' ').map((w) => w.charAt(0).toUpperCase() + w.substr(1, w.length - 1)).join(' ');
/**
 * Hello => hELLO
 *
 * @param text
...
StringsUtils.isEmpty = val => val == null || !(Object.keys(val) || val).length;
...
StringsUtils.size = val => Array.isArray(val)
    ...
    : val && typeof val === 'object'
        ...
        : typeof val === 'string'
            ? new Blob([val]).size
            : 0;
StringsUtils.queryStringToObject = (query = window.location.search.substring(1)) => {
    if (query.startsWith('?'))
 ...
 */
StringsUtils.toCurrency = (n, curr, LanguageFormat = undefined) => Intl.NumberFormat(LanguageFormat, {
    style: 'currency',
    currency: curr
}).format(n);
...
StringsUtils.words = (text, pattern = /[^a-zA-Z-]+/) => text.split(pattern).filter(Boolean);
/**
 ...
 * @param mask
 */
StringsUtils.mask = (cc, num = 4, mask = '*') => ('' + cc).slice(0, -num).replace(/./g, mask) + ('' + cc).slice(-num);
/**
 * pad('cat', 8); // '  cat   '
 ...
 * @param char
 */
StringsUtils.pad = (str, length, char = ' ') => str.padStart((str.length + length) / 2, char).padEnd(length, char);
/**
...
MathsUtils.average = (...nums) => nums.reduce((acc, val) => acc + val, 0) / nums.length;
/**
 ...
 * @param fn
 */
MathsUtils.averageBy = (arr, fn) => arr.map(typeof fn === 'function' ? fn : val => val[fn]).reduce((acc, val) => acc + val, 0) / arr.length;
/**
 * gcd(12,18) => 6
 *
 * @param arr
...
MathsUtils.primes = num => {
    let arr = Array.from({ length: num - 1 }).map((x, i) => i + 2), sqroot = Math.floor(Math.sqrt(num)), numsTillSqroot = Array.from({ length: sqroot - 1 }).map((x, i) => i + 2);
 ...
 * @param fn
 */
MathsUtils.maxBy = (arr, fn) => Math.max(...arr.map(typeof fn === 'function' ? fn : val => val[fn]));
MathsUtils.minBy = (arr, fn) => Math.min(...arr.map(typeof fn === 'function' ? fn : val => val[fn]));
/**
 ...
 * @param fn
 */
MathsUtils.sumBy = (arr, fn) => arr.map(typeof fn === 'function' ? fn : val => val[fn]).reduce((acc, val) => acc + val, 0);
/**
 ...
 * @param num
 */
MathsUtils.toSafeInteger = num => Math.round(Math.max(Math.min(num, Number.MAX_SAFE_INTEGER), Number.MIN_SAFE_INTEGER));
 ...
 * @param func
 */
PromiseUtil.promisify = func => (...args) => new Promise((resolve, reject) => func(...args, (err, result) => (err ? reject(err) : resolve(result))));
class Queue {
    constructor() {
        this.dataholder = [];
    }
